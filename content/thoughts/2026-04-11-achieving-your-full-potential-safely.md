...
- [Network Protocols](#network-protocols---how-ai-talks-to-the-world)
- [Sensor Data](#sensor-data---how-ai-perceives-the-physical-world)
- [Construction](#construction---where-it-all-converges-in-a-lagging-industry)
- [Cryptoeconomics](#cryptoeconomics---what-happens-when-you-give-an-agent-money)
...
It's not a simple question, especially for someone who lives to work, with appropriate breaks in between. My experience spans bleeding-edge tech that is still too early, and industries that haven't changed in decades. Evaluating the priorities of a given day or week is one thing. Planning out goals, milestones, and missions spans months and years. The AEC world is a good example of the latter: [Every Building You've Ever Been In Was Designed By Software Built in 1997](https://a16z.com/every-building-youve-ever-been-in-was-designed-by-software-built-in-1997/).
...
The hard problem in AI safety is not abstract alignment in the lab, but learning empirically from systems that are useful enough to deploy in high-stakes physical environments, while using faster-moving domains like cryptoeconomics to study emergent behavior before it reaches the real world. Across the path from APIs and RPCs to resources, sensor data, and peripherals, small failures compound into irreversible harm.

That means safety work should be organized by surface area and stakes: protocols first, then resources, then sensors, then actuators.

This post is my attempt to lay out what I'd focus on. It boils down to four convictions:
...
4. **AI acts on the world through peripherals and actuators.** A model that can see but not move is limited. The moment it can trigger tools, machines, robots, or other downstream systems, the safety envelope changes again.
